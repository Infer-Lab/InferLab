  ...
  "cases": [{
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        std::fs::write(
            &current,
            format!(
                r#"{{"schema_version":20,"id":"current","kind":"bench","status":"succeeded","started_unix_ms":1,"request_source":{{"kind":"dataset","catalog":{catalog},"preparation_attempt_id":"data-asset-1"}},"cases":[]}}"#,
            ),
        )?;
        std::fs::write(
            &historical,
        ...
        std::fs::write(
            &future,
            format!(
                r#"{{"schema_version":21,"id":"future","kind":"bench","status":"succeeded","started_unix_ms":1,"request_source":{{"kind":"dataset","catalog":{catalog},"preparation_attempt_id":"must-not-be-read"}},"cases":[]}}"#,
            ),
        )?;
