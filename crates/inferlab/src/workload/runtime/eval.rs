...
/// The fixed test image every vision smoke of this InferLab release embeds
/// ([[RFC-0004:C-MEASUREMENTS]]): a 96×64 PNG geometric pattern generated by
/// `spikes/generate-vision-smoke-image.py`, stable within one release. The
/// image content never participates in the smoke's success judgment.
const VISION_SMOKE_IMAGE_PNG: &[u8] = include_bytes!("../../../resources/vision-smoke.png");

/// The vision smoke request: one user message carrying the effective prompt
/// as one text part followed by one image_url part embedding the fixed image
/// as a data URI ([[RFC-0004:C-MEASUREMENTS]]).
#[derive(Serialize)]
struct OpenAiChatVisionRequest<'a> {
    model: &'a str,
    messages: [OpenAiChatVisionMessage<'a>; 1],
    max_tokens: u32,
    temperature: f64,
    stream: bool,
    n: u32,
...
#[derive(Serialize)]
struct OpenAiChatVisionMessage<'a> {
    role: &'static str,
    content: [OpenAiChatContentPart<'a>; 2],
}

#[derive(Serialize)]
#[serde(tag = "type", rename_all = "snake_case")]
enum OpenAiChatContentPart<'a> {
    Text { text: &'a str },
...
#[derive(Serialize)]
struct OpenAiSmokeRequestEvidence<'a, Body: Serialize> {
    method: &'static str,
    url: &'a str,
    body: &'a Body,
}

/// The smoke request body: the completions form, or the vision mode's single
/// chat message ([[RFC-0004:C-MEASUREMENTS]]). Untagged so each variant
/// serializes exactly as its own shape.
#[derive(Serialize)]
#[serde(untagged)]
enum OpenAiSmokeRequestBody<'a> {
    Completion(OpenAiCompletionRequest<'a>),
    ChatVision(OpenAiChatVisionRequest<'a>),
}

/// The data URI embedding the release-owned fixed test image.
fn vision_smoke_image_data_uri() -> std::borrow::Cow<'static, str> {
    use base64::Engine;
    std::borrow::Cow::Owned(format!(
        "data:image/png;base64,{}",
        base64::engine::general_purpose::STANDARD.encode(VISION_SMOKE_IMAGE_PNG)
    ))
}
...
pub(super) fn run_openai_smoke(
    ...
    // The vision mode routes to chat completions; everything else about the
    // request shape — the effective prompt, the maximum-token limit, zero
    // temperature, non-streaming — is shared with the completions smoke.
    let (url, body) = if *vision {
        let url = format!(
            "{scheme}://{}:{}{}",
            plan.endpoint.host, plan.endpoint.port, plan.endpoint.chat_completions_path
        );
        let body = OpenAiSmokeRequestBody::ChatVision(OpenAiChatVisionRequest {
            model: &plan.model.served_name,
            messages: [OpenAiChatVisionMessage {
                role: "user",
                content: [
                    OpenAiChatContentPart::Text { text: prompt },
                    OpenAiChatContentPart::ImageUrl {
                        image_url: OpenAiImageUrl {
                            url: vision_smoke_image_data_uri(),
                        },
                    },
                ],
            }],
        ...
        let body = OpenAiSmokeRequestBody::Completion(OpenAiCompletionRequest {
            model: &plan.model.served_name,
            prompt,
            max_tokens: *max_tokens,
            temperature: 0.0,
    ...
    match response {
        Ok(response) => {
            ...
            if !(200..300).contains(&response.status) {
                // Keep a body-read diagnostic when one was already recorded:
                // the HTTP status is the headline, not a replacement.
                let failure = format!("OpenAI smoke request returned HTTP {}", response.status);
                error = Some(match error {
                    Some(body_error) => format!("{failure}; {body_error}"),
                    None => failure,
                });
            } else if let Some(body) = body {
                let validation = if *vision {
                    validate_openai_chat_vision_body(&body)
                } else {
                    validate_openai_completion_body(&body)
                };
                match validation {
                    Ok(choices_count) => {
...
/// The vision smoke's success judgment: the completions failure conditions
/// with the first choice's `message.content` string in place of `text`
/// ([[RFC-0004:C-MEASUREMENTS]]). The image content never participates.
pub(super) fn validate_openai_chat_vision_body(body: &[u8]) -> Result<usize, String> {
    let value: Value = serde_json::from_slice(body)
        .map_err(|error| format!("OpenAI chat completion response was not valid JSON: {error}"))?;
    let object = value
        .as_object()
        .ok_or_else(|| "OpenAI chat completion response was not a JSON object".to_owned())?;
    let choices = object
        .get("choices")
        .and_then(Value::as_array)
        .ok_or_else(|| "OpenAI chat completion response had no choices array".to_owned())?;
    let first = choices
        .first()
        .and_then(Value::as_object)
        .ok_or_else(|| "OpenAI chat completion response choices array was empty".to_owned())?;
    first
        .get("message")
        .and_then(Value::as_object)
        ...
        .and_then(Value::as_str)
        .ok_or_else(|| {
            "OpenAI chat completion response first choice had no string message.content".to_owned()
        })?;
    Ok(choices.len())
}
