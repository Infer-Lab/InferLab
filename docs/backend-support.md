...
The version correspondence for the current product line — `inferlab --version`
states the adapter protocol version a binary speaks:

| InferLab product | Adapter protocol | Adapter SDK | Framework integrations |
| --- | --- | --- | --- |
...
| current main (0.14.3 development line) | v11 | next Adapter SDK release (0.10.0 speaks v10) | next integration releases (0.9.0 speaks v10) |

Each framework integration package pins its Adapter SDK exactly, so pinning the
integration versions above is sufficient. Retained qualification records from
earlier exact package versions remain historical evidence; they do not qualify
these candidates unless a cell explicitly names a protocol-v7 record.
...
| Vision smoke Eval | Qualified on Qwen2.5-VL-7B-Instruct SM120 TP1: the vision smoke POSTed one chat-completions request carrying the release-owned fixed test image as an `image_url` part, the served model's answer described the image content, and the schema-20 record preserves the vision selection with request and response artifacts | Supported: same native smoke path; real VLM route qualification is pending | Supported: same native smoke path; real VLM route qualification is pending | Supported: same native smoke path; real VLM route qualification is pending | Supported: same native smoke path; real VLM route qualification is pending |
| Image-decorated random Bench | Qualified on Qwen2.5-VL-7B-Instruct SM120 TP1 for both the synthetic noise supply (two 512x384 images per request) and a digest-bound operator directory (one image per request, shuffle-cycle sampling): every request carried `image_url` content parts on the chat-completions route, and backend-observed prompt tokens reconciled the 512-token text target plus per-image tokens exactly (1020 and 766) | Supported: same runner-side decoration; real VLM route qualification is pending | Supported: same runner-side decoration; real VLM route qualification is pending | Supported: same runner-side decoration; real VLM route qualification is pending | Supported: same runner-side decoration; real VLM route qualification is pending |
| SemiAnalysis AgentX trace replay | Qualified for the immutable 062126 256k profile on direct `single`: Qwen3-30B-A3B-Instruct-2507 at TP2 on two NVIDIA RTX PRO 6000 Blackwell Server Edition GPUs completed the profile-owned 600-second cache-pressure warmup and 900-second profiling window at root-tree concurrency 1 (a record predating the profile's ADR-0044 change, which replaced the fixed-duration warmup with a per-lane request count); the scenario-valid record preserved 696 error-free warmup requests, 141 profiling requests with one ordinary failure, source coordinates, cache-bust observations, branch statistics, transport metrics, and verified cleanup. The full-context profile is Supported but Unqualified | Supported by the shared runner; real SGLang route qualification is pending | Supported by the shared runner; real TensorRT-LLM route qualification is pending | Supported by the shared runner; real TokenSpeed route qualification is pending | Supported by the shared runner; real Specialized Engine route qualification is pending |
| Synthetic acceptance overlay | Supported: the typed `synthetic_acceptance` declaration (explicit value or digest-pinned golden acceptance-length curve) is overlaid onto the operator's `--speculative-config` as synthetic rejection sampling, resolving the curve-form acceptance length against the draft count parsed from that configuration; real-route qualification is pending | Supported: the declaration renders the `SGLANG_SIMULATE_ACC_*` environment over the operator's `--speculative-*` flags (requiring a framework build that honors those variables); real-route qualification is pending | Supported: the declaration renders the forced-acceptance environment with the documented off-by-one adjustment over the operator's llm-api `speculative_config`; real-route qualification is pending | —: the integration rejects a declared synthetic acceptance overlay with a typed planning error | —: the integration rejects a declared synthetic acceptance overlay with a typed planning error |
| Auxiliary model weights (`draft-model`) | Supported: the server-level `auxiliary_models` declaration resolves the referenced registry model's per-machine weight locator onto every model-rank allocation, and the integration splices it as the `model` key of the operator's `--speculative-config` JSON, which must exist and must not already set that key; real-route qualification is pending | Supported: the declaration renders `--speculative-draft-model-path` alongside the operator's speculative flags, requiring `--speculative-algorithm` as the splice target and rejecting an operator-spelled draft path; real-route qualification is pending | Supported: the declaration splices the resolved locator into the `speculative_model` key of the operator's llm-api `speculative_config`, which must exist and must not already set that key; real-route qualification is pending | —: the integration rejects a declared auxiliary model with a typed planning error | —: the integration rejects a declared auxiliary model with a typed planning error |
