...
For profiling enablement, deadline fields, Nsight Systems settings, escapes,
and environment syntax, read
...
Each model-rank target owns one Nsight Systems session. InferLab enumerates the
expected semantic windows, arms every selected target, opens the framework
range before the bound measurement phase, closes it after client completion or
failure, finalizes collection, and verifies required report coverage.
...
bounded so process exit and operator interruption stay observable.
...
A failed framework close action is preserved but does not by itself discard a
complete trace. Final report coverage decides capture success; missing coverage
fails the profile and retains both control and coverage evidence.
