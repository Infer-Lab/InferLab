...
mode="${1:-check}"
adapter_schema="${root}/protocol/schema/adapter-protocol-v10.schema.json"
...
cargo run --quiet --locked --manifest-path "${root}/Cargo.toml" \
  ...
  "${temporary}/schema/adapter-protocol-v10.schema.json" \
...
case "${mode}" in
  generate)
    ...
    cp "${temporary}/schema/adapter-protocol-v10.schema.json" "${adapter_schema}"
    ...
    ;;
  check)
    cmp "${temporary}/schema/adapter-protocol-v10.schema.json" "${adapter_schema}"
